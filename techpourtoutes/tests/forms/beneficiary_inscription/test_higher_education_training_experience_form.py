...
def _valid_data(higher_ed_school, **overrides):
    return {
...
def test_save_creates_a_training_experience_for_the_current_school_year(
    ...
    form = BeneficiaryHigherEducationTrainingExperienceForm(data=_valid_data(higher_ed_school))
...
@pytest.mark.django_db
def test_form_rejects_a_secondary_level(higher_ed_school):
    form = BeneficiaryHigherEducationTrainingExperienceForm(
        data=_valid_data(higher_ed_school, level=TrainingExperience.Level.TERMINALE)
    )
...
@pytest.mark.django_db
def test_form_rejects_an_unknown_higher_ed_school(higher_ed_school):
    form = BeneficiaryHigherEducationTrainingExperienceForm(
        data=_valid_data(higher_ed_school, higher_ed_school_id="not-a-uuid")
    )
...
@pytest.mark.django_db
def test_form_requires_a_higher_ed_school(higher_ed_school):
    form = BeneficiaryHigherEducationTrainingExperienceForm(
        data=_valid_data(higher_ed_school, higher_ed_school_id="")
    )
...
def test_form_ignores_a_level_answered_in_the_other_branch():
    ...
    form = BeneficiaryHigherEducationTrainingExperienceForm(
        initial={"level": TrainingExperience.Level.TERMINALE}
    )
